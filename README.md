...
The browser version is a faithful port of the Java app. Same game modes, same levels, same scoring formula — running entirely in vanilla JavaScript with zero dependencies.
...
| 💾 **Persistent Stats** | High score and best streak saved across sessions |
...
| JavaFX | 21 | Desktop UI — Timeline, FadeTransition, ParallelTransition |
...
The `CountSystem` interface (Strategy pattern) means adding a new system requires exactly one new class — no other code changes:
